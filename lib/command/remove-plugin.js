...
export const removePlugin = async (
  ...
  const pluginDir = path.join(presentationRoot, 'plugins', pluginName);

  const pluginDirExists = existsSync(pluginDir);

  const pluginDirStat = pluginDirExists ? await stat(pluginDir) : null;

  if (!presJson?.plugins?.[pluginName] && (!pluginDirExists || !pluginDirStat.isDirectory())) {
  ...
  if (pluginDirExists && pluginDirStat.isDirectory()) {
