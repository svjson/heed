...
export const ensurePluginDir = async (presentationRoot) => {
  const pluginDir = resolvePluginDir(presentationRoot);
...
export const resolvePluginSource = async (pluginName, source, heedRoot) => {
  if (!source) {
    ...
    if (!plugins[pluginName]) {
      throw new Error(
        `Unknown plugin '${pluginName}'. If the plugin exists, you must provide a source.`,
      );
    }
...
export const verifyNotInstalled = (presentationRoot, presJson, pluginName) => {
  if (
    presJson?.plugins?.[pluginName] &&
    isPluginPresentInFolder(presentationRoot, pluginName)
  ) {
    throw new Error(
      `Plugin '${pluginName}' is already installed in this presentation.`,
    );
  }
};
 ...
 * @return {Promise<Object>} - An object containing the installed path and default
 */
export const installFromPath = async (
  presentationRoot,
  pluginName,
  source,
  bySymlink = false,
  ...
  if (pluginDotJson.pluginId !== pluginName) {
    throw new Error(
      `Plugin IDs do not match. Provided: '${pluginName}' vs plugin.json: '${pluginDotJson.pluginId}'.`,
    );
  }
  ...
  return {
    ...
    defaultConfig: pluginDotJson.defaultConfig ?? {},
  };
};

/**
 * Load plugin definitions for the plugins defined in the presentation.
 *
 * @param {string} presentationRoot - The root directory of the presentation.
 * @param {Object} presentation - The presentation JSON object.
 * @return {Promise<Object>} - An object containing the loaded plugin definitions.
 */
export const loadPluginDefinitions = async (presentationRoot, presentation) => {
  const pluginDir = resolvePluginDir(presentationRoot);
  const pluginIds = Object.keys(presentation?.plugins ?? {});
...
/**
 * Attach plugin hooks to slideIr.
 *
 * The provided presentation must have loaded plugin definitions or else
 * this function is a no-op.
 *
 * @param {Object} ir - The intermediate representation of the slide.
 * @param {Object} presentation - The presentation JSON object with loaded plugins.
 *
 * @return {void}
 */
export const applyHooks = (ir, presentation) => {
  if (!presentation) return;
  if (!presentation.plugins) return;
  const sectionHasMatchingComponent = (section, matcher) => {
    for (const block of section) {
      if (matcher(block.type)) return true;
      if (Array.isArray(block.children)) {
        if (sectionHasMatchingComponent(block.children, matcher)) {
          return true;
        }
      }
    }
  ...
  Object.entries(presentation.plugins).forEach(([pluginId, plugin]) => {
    Object.entries(plugin.definition?.provides?.hooks ?? {}).forEach(
      ([hookType, hook]) => {
        ...
        if (shouldApply) {
          ...
          hooks[hookType] ??= {};
          hooks[hookType][`${pluginId}:${hookType}`] = {};
        }
      },
    );
  });
