...
import { applyHooks } from './plugin.js';
 ...
 *                 - `presentation`: The presentation JSON, used for context during
 *                    parsing to resolve plugin-specific constructs in the slide.
 *
...
export const loadSlide = async (presentationRoot, slidePath, opts = {}) => {
  ...
  try {
    const ir = parseHeedFile(data, opts.presentation);
    ...
    applyHooks(ir, opts.presentation);
...
export const createSlide = async (filePath, frontMatter) => {
  ...
  const divider = '-'.repeat(
    Math.max(lines.reduce((max, line) => Math.max(max, line.length), 0)),
  );
