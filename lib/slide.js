...
import { makeParseErrorSlide } from './parse-error.js';
 ...
 */
export const loadSlide = async (presentationRoot, path, useErrorSlide) => {
  ...
  try {
    ...
    if (useErrorSlide) {
      return makeParseErrorSlide(e, data, fileName.substring(presentationRoot.length));
    }
