...
/**
 * Parse the %values attribute value of a %for-macro block.
 *
 * This function can handle both comma-separated lists and ranges
 * in the form of "1..3".
 *
 * @param values - The values string from the %for-macro block.
 * @returns An array of parsed values.
 */
const parseLoopValues = (values) => {
  const match = values.trim().match(/^(\d+)\.\.(\d+)$/);
  if (match) {
...
export const expandForBlock = (block) => {
  ...
  const items = parseLoopValues(values);
