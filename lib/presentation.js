...
export const getIndex = async (presentationRoot, presentation) => {
  ...
  if (index.slide) {
    ...
    const [_fileName, type] = await findSlideFile(presentationRoot, path.join(slidePath, presentation.slide.id));
  ...
  if (Array.isArray(index.slides)) {
    for (const slide of index.slides) {
      const slidePath = 'slides';
      const [_fileName, type] = await findSlideFile(presentationRoot, path.join(slidePath, slide.id));
  ...
  if (Array.isArray(index.sections)) {
    for (const section of index.sections) {
      ...
      for (const slide of section?.slides ?? [])  {
        ...
        const [_fileName, type] = await findSlideFile(presentationRoot, path.join(slidePath, slideId));
...
export const findRootRecursive = async (dir, maxDepth=5) => {
  ...
  for (const ent of contents) {
    if (ent.isDirectory()) {
      const root = await findRootRecursive(path.join(dir, ent.name), maxDepth-1);
      if (root) {
        return root;
      }
    }
