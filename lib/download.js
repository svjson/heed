...
export const isUrl = (urlOrPath) => {
  try {
    ...
    return !!url.protocol && url.protocol !== 'file:';
  } catch {
    return false;
  }
};
...
export const downloadFile = async (url, destPath) => {
  ...
  if (existsSync(destPath)) {
    const pathStat = statSync(destPath);
    if (pathStat.isDirectory()) {
      const _url = new URL(url);
      filePath = path.join(destPath, _url.pathname.split('/').at(-1));
    }
  }
...
/**
 * Prepare a temporary directory for downloading files.
 *
 * @return {Promise<string>} - A promise that resolves to the path of
 *                             the temporary directory.
 */
export const prepareDownloadDirectory = async () => {
  const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'heed-download-'));
  return tmpDir;
};
...
export const unzipTo = async (zipPath, targetFolder) => {
  targetFolder =
    targetFolder ??
    (await mkdtemp(path.join(os.tmpdir(), 'heed-plugin-unzip-')));
