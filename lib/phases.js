...
/**
 * Identify and extract phase attributes, returning a tuple containing
...
export const extractPhaseDirectives = (block) => {
  ...
  Object.entries(block.attributes ?? {}).forEach(([attr, value]) => {
  ...
  return [attributes, phaseDirectives];
}
...
/**
 * Extracts the phase directives from the block and applies them to the `phases` key
 * of the slide, removing them from the `attributes` object.
 */
export const applyBlockPhaseAttributes = (blockIr, slideIr) => {
  const [attributes, phaseAttributes] = extractPhaseDirectives(blockIr);
  blockIr.attributes = attributes;
